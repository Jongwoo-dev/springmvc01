...
public class Control10 {
  ...
  public String test01() {
    // DispatcherServlet(프론트 컨트롤러)는 요청 핸들러(페이지컨트롤러의 메서드)가
   ...
   */
  @RequestMapping(path="/ex5/test03", produces="text/html;charset=UTF-8")   // value나 path 나 같다.
  @ResponseBody
   ...
   *    뷰 리졸버로 보내지 않고 이 객체에 들어있는 콘텐츠와 헤더를 가지고
   *    클라이언트로 바로 출력한다.
   */
  @RequestMapping(path="/ex5/test04")   // value나 path 나 같다.
  @ResponseBody
  public ResponseEntity<String> test04() {
    HttpHeaders headers = new HttpHeaders();
    headers.set("Content-Type", "text/html;charset=UTF-8");
    return new ResponseEntity<String>(
        "<html><body><h1>test03</h1><p>ABC123가나다</p></body></html>",
        headers, 
        HttpStatus.OK);
  }
}
